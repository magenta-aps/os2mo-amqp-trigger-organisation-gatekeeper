...
- Integration tests require **starting the stack** if it isn't running
  - Clone os2mo if it isn't in your environment: https://github.com/OS2mo/os2mo
  - Start the os2mo stack according to its README
  - Start this integration's stack with `docker compose up -d --build`
- Run integration tests via: `docker compose run --rm orggatekeeper pytest <test-path>`
- os2mo not running is not an excuse to skip integration tests.
  - You must make an effort to start os2mo, or clone it if needed, even if doing so is time consuming, or
    requires you to write to a directory outside the current working directory.
