                ...
                "objects": [
                    {
                        "current": {
                            ...
                            "engagements": [
                                {
                                    "uuid": uuid4(),
                                    "engagement_type": {"name": engagement_type},
                                }
                            ],
                            "associations": [],
                            "children": [],
...
async def test_update_line_management_hidden(
    ...
    fetch_org_unit.assert_called_once_with(gql_client, uuid)
    model_client_shim = context["model_client_shim"]
    model_client_shim.edit.assert_called_once_with(
        context["graphql_client"],
        org_unit.copy(
            update={
...
async def test_update_line_management_line(
    ...
    if not changes:
        ...
        fetch_org_unit.assert_called_once_with(gql_client, uuid)
        model_client_shim.edit.assert_not_called()
